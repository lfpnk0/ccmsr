...
function httpReq(params){ //params = {method,url,auth,req,callback}
  ...
  filename = 'httpReq'+new Date().getTime();
  params.responsePath = TEMP+'/'+filename+'.resp';

  if (!params.hasOwnProperty('method')){
    var f = fso.CreateTextFile(params.responsePath, true);
    f.writeLine('{"error":{"code":000},"response":{"message":"No Method provided to httpReq()"}}');
    f.Close();
  }
  if(params.method == "PUT" || params.method == "POST" || params.method == "DELETE"){
    if (!params.hasOwnProperty('req')){
      var f = fso.CreateTextFile(params.responsePath, true);
      f.writeLine('{"error":{"code":000},"response":{"message":"No Request provided to httpReq()"}}');
      f.Close();
    }
  }
  if (!params.hasOwnProperty('url') || params.url === 'undefined'){
    var f = fso.CreateTextFile(params.responsePath, true);
    f.writeLine('{"error":{"code":000},"response":{"message":"No URL Provided to httpReq()"}}');
    f.Close();
  }
  else{
